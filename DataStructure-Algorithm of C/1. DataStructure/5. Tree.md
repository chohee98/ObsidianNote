...
- **노드 표현 방법**: N-링크(N-Link), 왼쪽 자식-오른쪽 형제(Left Child-Right Sibiling) -> 부모와 자식, 형제 노드를 연결 짓는 방법
	- N-링크: 차수(자식 노드이 수)가 노드마다 달라지는 트리에는 적용하기 힘들다는 단점
...
![[5-2. LCRSTreeSesult1.png]] ![[5-3. LCRSTreeSesult2.png|600]]
...
2. 중위 순회
	...
	- 응용되는 대표 사례는 **수식 트리** (ex. `(1*2) + (7-8)` ) -> 중위 표기식![[5-5. 중위 순회.png|300]]
3. 후위 순회
	- 오른쪽 하위 트리-왼쪽 하위 트리-뿌리 노드 순으로 방문
	- 위의 중위 순회를 후위 표기식으로 출력하면 `12*78-+` ![[5-6. 후위순회.png|330]]
...
![[5-7. BinaryTreeResult1.png]]     ![[5-8. BinaryTreeResult2.png]]
