...
1. 얕은 복사(Shallow Copy)
	- <mark style="background: #824CB496;">멤버 데이터를 비트열 단위로 '똑같이' 복사</mark> (메모리 영역 값(주소만)을 그대로 복사)
...
2. 깊은 복사(Deep Copy)
	- <mark style="background: #824CB496;">어떤 클래스의 멤버 데이터가 참조(주소) 값이라면, 데이터를 새로 만들어준다</mark> (원본 객체가 참조하는 대상까지 새로 만들어서 복사)
	- 포인터는 주소값 바구니니까 *새로운 객체를 생성*해서 상이한 객체를 가리키는 상태로 만들기
	- 어떤 클래스의 <mark style="background: #FFAB45CF;">멤버 데이터에 참조나 포인터가 들어가게 되면</mark>, 컴파일러가 기본적으로 만들어 주던 복사 생성자나 복사 대입 연산자를 그대로 사용하지 않고 **오버로딩**을 이용하여 <mark style="background: #FFAB45CF;">명시적으로 생성자 만들기</mark> (컴파일러가 자동으로 만들어 줬을 때는 메모리 값을 그대로 똑같이 복사해 줬다면 복사 될 때 어떤 행동을 해야하는 지를 직접 정리) 
...
- 명시적 복사 생성자 Steps Rule (모든 것을 우리가 컨트롤하겠다는 의미)
	...
	3. (다른 걸 호출하라고 명시하지 할 경우) `Knight(const Knight& knight) : Player(knight), _pet(knight._pet)` 처럼 직접 챙겨주지 않으면 명시적으로 넣어준 부분은 복사가 됐겠지만 나머지 부분은 복사가 안되어 있는 상태 (초기화가 안돼 있는 문제) -> #암기 
