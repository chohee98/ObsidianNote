...
2. 참조 타입 반환
	...
	- 포인터와 유사하게 주소값을 다루기 때문에 만들어진 형태 만을 빠르게 해석 (의미는 포기하고 데이터만 유지하는 셈)
...
2. 불안전한 변환
	- 특징) 의미가 항상 100% 일치한다고 보장하지 못하는 경우 ➡️ 데이터 분실 위험
...
int main()
{
	...
	// 연관 없는 클래스 사이의 참조 타입 변환
	{
		...
		// 즉 참조 타입 반환은 형태만을 가지고 통과시켜 주기 때문에 실제 사실이 아니더라도 통과시켜주는 반면
		// 타입 변환 생성자는 진짜 객체를 대상으로 데이터를 통으로 복사해서 넣어주는 형식이기 때문에 엄격하게 체크 -> 타입 변환 생성자, 타입 변환 연산자를 이용해야 함
	}
	return 0;
}
```
...
- 함수한테 넘겨줄 때도 복사 방식으로 넘겨주는 것(`TestItem(Item item)`)과 포인터 방식으로 넘겨주는 것(`TestItemPtr(Item* item)`)의 차이 -> 복사 생성자 & 소멸자 호출 여부!
```cpp
class Item
{
public:
...
int main()
{
	// 복습 (메모리 구조 그리기 - 차이 암기 🔥)
	{
		...
		// 점점 가용 메모리가 줄어들어서 Crash, 프로그램이 터질 수 있음..
		delete item2;
	}
