...
- **결론**
	- <mark style="background: #FF898996;">포인터 사이의 타입 변환(캐스팅)을 할 때는 매우 조심해야 한다!</mark> -> 여러 클래스들을 최상위 타입으로 바꿔서 저장하고 싶을 때 (인벤토리 케이스처럼) 부모 타입에서 자식 클래스로 변환도 하고 반대로 하면서 왔다 갔다 하는 일이 자주 발생
