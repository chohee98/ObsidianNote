...
### 비트열 재구성 여부
1. 값 타입 반환
	- 특징) _의미를 유지_ 하기 위해서, 원본 객체와 다른 비트열로 재구성 (논리적으로 생각)
2. 참조 타입 반환
	- 특징) 비트열을 재구성하지 않고, _관점_ 만 바꾸는 것
	- 만들어진 형태만을 빠르게 해석 (의미는 포기하고 데이터만 유지하는 셈)
...
### 프로그래머 의도에 따라 분류 
1. 암시적 변환 #암기
	- 특징) 이미 알려진 타입 변환 규칙에 따라서 _컴파일러가 자동_ 으로 타입 변환
...
int main()
{
	...
	{
		Knight knight;
		// 암시적으로는 안되지만 명시적 참조를 하면 얘가 엉뚱하게 메모리를 초과해서 사용하는 문제 발생
		// 나이트는 int 4바이트짜리, 도그는 8바이트짜리로 만들어줬기 때문
		// 그럼 왜 이 문법이 통과되는 걸까?
		Dog& dog = (Dog&)knight;
		dog._cuteness = 12;
		...
		// 형식 자체는 유효 -> 포인터는 이런 상황이 비일비제하게 일어남 (일단은 여유롭게 통과시켜 줌)

		// 반면 타입 변환 생성자는 데이터를 통으로 복사해서 넣어주는 형식이기 때문에 (객체 대상 다룸)
...
1. 상속 관계 클래스의 값 타입 변환
	- 불독에서 개로 넘어가려고 했으면 일부 데이터가 짤린 채로 나머지 필요한 데이터만 적절히 넣어 줘서 변환 가능
...
int main()
{
	...
	{
		//Dog dog;
		//BullDog bulldog = dog;	// 변환 실패 (모든 개가 불독은 아님)

		// 위에 타입 변환 생성자가 있으면 기본 생성자는 막히기 때문에 타입 변환 생성자를 없애거나
		// 기본 생성자를 따로 추가해 줘야 함		
		// 개가 가지고 있는 정보를 불독도 가지고 있기 때문에 변환이 융통성은 있음
		// _french 여부만 짤린 상태로 나머지 정보만 dog에 들어감
		BullDog bulldog;
		Dog dog = bulldog;
	}
	...
	// 상속 관계 클래스의 참조 타입 변환
	{
		...
		BullDog bulldog;
		Dog& dog = bulldog;	// (별 위화감 없이) 암시적으로 해도 컴파일 성공
	}
	return 0;
}
```
...
## 복습 
  #암기
- <mark style="background: #FFAB45CF;">(포인터) vs (일반 타입) 변수 차이</mark> 이해하기 
- 함수한테 넘겨줄 때도 복사 방식으로 넘겨주는 것과 포인터 방식으로 넘겨주는 것의 차이 -> 복사 생성자 & 소멸자 호출 여부!
```cpp
class Item
{
public:
...
{ // item도 임시 뗌빵용이지만 객체이기 때문에 복사 생성자가 호출되고 소멸될 때 소멸자 호출
}

void TestItemPtr(Item* item)
{ // 포인터 또는 참조를 넘겨줄 때는 어떤 생성자나 소멸자가 따로 호출되지 않음 (살아있는 생명체로 인정을 안해주는 느낌?)
}
...
int main()
{
	// 복습 (메모리 구조 그리기 - 차이 암기)
	{
		...
		TestItem(item);  // TestItem(Item item), sizeof(item) = 4104		
		TestItem(*item2);  // sizeof(item2) = 4
...
- **결론**
	- <mark style="background: #824CB496;">포인터 사이의 타입 변환(캐스팅)을 할 때는 매우 조심해야 한다! </mark>-> 여러 클래스들을 최상위 타입으로 바꿔서 저장하고 싶을 때 (인벤토리 케이스처럼) 부모 타입에서 자식 클래스로 변환도 하고 반대로 하면서 왔다 갔다 하는 일이 자주 발생
	- <mark style="background: #FFAB45CF;">부모-자식 관계에서 부모 클래스의 소멸자에는 까먹지 말고 virtual을 붙이자!!</mark> -> 이렇게 하지 않으면 소멸자가 일부만 실행되어 깔끔하게 지워줘야 하는 메모리들이 날아가지 안는다거나 하는 문제 발생
	- <mark style="background: #824CB496;">(이유!)</mark> "상속 관계에 의해 함수를 재정의 해봤자 어떤 타입에 따라 가지고 어떤 한 애만 자동으로 골라가지고 실행이 되기 때문에 virtual을 붙여주게 되면 가상함수 테이블이 만들어지게 되면서 실제 객체가 뭐로 만들어젔나에 따라가지고 그 함수를 찾아서 호출" #암기
...
int main()
{
	...
	// 인벤토리에서 뿐만 아니라 플레이어, npc, 몬스터 등도 creature 생명체로 관리해서
	...
	for (int i = 0; i < 20; i++)
	{
		...
		// 자식 클래스의 생성자를 호출하기 전에 부모 클래스의 생성자 호출 하지만!
