...
2. **초기화 여부**
	...
	- <mark style="background: #FF898996;">참조 타입은 nullptr의 개념이 없다</mark>
	- 없는 값을 표현해야 한다면 포인터를 써서 null 체크, 바뀌지 않고 읽는 용도 readonly만 사용하면 <mark style="background: #0E4F9FA6;">conset ref&</mark>, 그 외 일반적인 경우 ref(값 변경이 일어날 경우 <mark style="background: #0E4F9FA6;">OUT ref&</mark>을 붙인다 - 언리얼에서 자주 사용)
```cpp
...
- `const int &ref = 30;` 이렇듯 상수를 const 참조자로 참조할 경우 임시 변수를 만들어서 상수를 메모리 공간에 임시적으로 저장(행을 바꿔도 소멸시키지 않는다)하기 때문에 const 참조자가 예외적으로 상수 참조 가능
