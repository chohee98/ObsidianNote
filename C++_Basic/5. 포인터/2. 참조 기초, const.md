...
2. **초기화 여부**
	...
	- 없는 값을 사용하는 게 도움이 되는 경우도 있다 -> 예를 들어 몬스터를 찾는 함수를 만들었을 때 `StatInfo* FindMonster()` 라는 함수를 만들었을 때 Heap 영역에서 뭔 가를 찾는데 찾지 못했을 때 *return nullptr*과 같이 사용 가능
	...
	- 결론) 없는 값을 고려해야 한다면 포인터를 써서 null 체크, 바뀌지 않고 읽는 용도 readonly만 사용하면 <mark style="background: #0E4F9FA6;">const ref&</mark>, 그 외 일반적인 경우 ref(값 변경이 일어날 경우 <mark style="background: #0E4F9FA6;">OUT ref</mark>을 붙인다 - 언리얼에서 자주 사용)
#코드
```cpp
struct StatInfo
{
