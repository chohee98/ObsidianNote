...
- Knight(자식) 클래스를 생성하면 부모 클래스의 생성자와 자식 클래스의 생성자 모두 호출
	- (호출 순서) <mark style="background: #FF898996;">부모 클래스 생성자 호출 -> 자식 클래스 생성자 호출 -> 자식 클래스 소멸자 호출 -> 부모 클래스 소멸자 호출</mark>
	...
	- 꼭 부모의 기본 생성자만 호출 할 수 있는 건 아님 `Knight(int stamina) : Player(100)`
- 예시
...
- 몰라도 되는 것은 깔끔하게 숨기겠다 그 이유는?
	1. 정말 위험하고 건드리면 안되는 경우
		...
		- <mark style="background: #FF898996;">맴버 변수를 private로 선언하고 접근 가능한 함수를 만드는 것이 main에서 몇 줄에 걸쳐서 데이터 값을 변경하는 것보다 안정적</mark> (멤버 변수에 접근할 유일한 방법)
...
- 바인딩(Binding): 어떤 함수를 호출해야 하는지 함수랑 주소랑 매핑 해줘서 연결
	- <mark style="background: #FF898996;">정적 바인딩</mark>(Static Binding): 컴파일 시점에 결정
		- 일반 함수는 "정적 바인딩"을 사용
	...
	- <mark style="background: #FFAB45CF;">동적 바인딩</mark>(Dynamic Binding): 실행 시점에 결정
		- 동적 바인딩을 원한다면 #가상함수 (Virtual Function)
			...
			- `MoveKnight(&p)` 가상 함수 선언을 않하면 player의 Move() 함수 실행하지만 가상 함수 선언을 해주면 knight의 Move() 함수 실행 (일반 함수로 만들어 줄 경우 포인터로 통합해서 관리해 줄 경우 포인터에 해당하는 일반 함수가 호출)
