...
- 바인딩(Binding): 어떤 함수를 호출해야 하는지 함수랑 주소랑 매핑 해줘서 연결
	...
	- <mark style="background: #FFAB45CF;">동적 바인딩</mark>(Dynamic Binding): 실행 시점에 결정
		- 동적 바인딩을 원한다면 #가상함수 (Virtual Function)
			...
			- (결론) 가상 함수 테이블에 해당 테이블의 주소가 첫 번째 인자 주소가 됨. 매개 변수 위쪽 영역에 가상 함수 테이블을 가리키는 주소가 낑겨 들어감 (원본 객체에 맞는 함수를 호출해줌)
			- #순수가상함수: 구현은 없고 '인터페이스'만 전달하는 용도로 사용하고 싶을 경우
				- 우리가 입장할 때 기사나 궁수나 법사 중에서 선택해서 들어가야 되지 `Player p;` 처럼 존재해서는 안됨.
				- 순수 가상 함수가 1개 이상 포함되면 바로 추상 클래스로 간주 -> 직접적으로 객체를 만들 수 없게 됨
