...
- 바인딩(Binding)
	...
	- <mark style="background: #FF898996;">동적 바인딩</mark>(Dynamic Bindinf): 실행 시점에 결정
	- 일반 함수는 정적 바인딩을 사용
	- 동적 바인딩을 원한다면 #가상함수 (Virtual Function)
		- 가상 함수는 재정의를 하더라도 가상 함수!
		- `MoveKnight(&p)` 가상 함수 선언을 안하면 player의 Move() 함수 실행하지만 가상 함수 선언을 해주면 knight의 Move() 함수 실행 (일반 함수로 만들어 줄 경우 포인터로 통합해서 관리해 줄 경우 포인터에 해당하는 일반 함수가 호출)
		- 그럼 어떻게 knight에서 player의 주소 이상의 정보에 접근하느냐?
		- 실제 객체가 어떤 타입인지 어떻게 알고 알아서 가상 함수를 호출 해준걸까?
		- 정답은 **가상 함수 테이블** (vftable)
		- `.vftable []` 4바이트(32)  8바이트(64)
		- 가상 함수 테이블에 해당 테이블의 주소가 첫 번째 인자 주소가 됨. 매개 변수 위쪽 영역에 가상 함수 테이블을 가리키는 주소가 낑겨 들어감 (원본 객체에 맞는 함수를 호출해줌)
		- 순수 가상 함수: 구현은 없고 '인터페이스'만 전달하는 용도로 사용하고 싶을 경우, 순수 가상 함수가 1개 이상 포함되면 바로 추상 클래스로 간주 -> 직접적으로 객체를 만들 수 없게 됨
...
public:
	...
	void VMove() { cout << "Move Knight!" << endl; }	// 가상함수 재정의
public:
	int _stamina;
};
...
int main()
{
	// Player p;
	//MovePlayer(&p);
	// 가상함수 사용을 안할 경우 플레이어는 기사가 아니기 때문에 에러 (하지만 역(MovePlayer(&k))은 성립)
	//MoveKnight(&p); 

	Knight k;
	//MoveKnight(&k);
	MovePlayer(&k); // player의 Move() 함수 실행
	//k.Move();
